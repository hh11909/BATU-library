import { Author, Book, Factory } from "./utils"

const _API = 'http://127.0.0.1:8080'


const El: Factory = function(tagName, attrs, children) {
...
const registerUser = async (e: SubmitEvent) => {
  ...
  if (
    email && pass && confirmPass && firstName && lastName && studentId && gender
  ) {
...
const registerUserInfo = async (e: SubmitEvent) => {
  e.preventDefault()
  const department = getValueById('department')
  const phoneNumber = getValueById('phoneNumber');
  const img = (document.getElementById('profileImage') as HTMLInputElement).files[0]
  if (
    img && phoneNumber && department
  ) {
    try {
      const res = await fetch(`${_API}/api/signup`, {
        method: 'POST',
